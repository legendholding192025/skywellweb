...
export async function POST(request: Request) {
  try {
    ...
    const apiEndpoint = 'https://lmmta.legendholding.com/CRM_IS/rest/RESTAPIDealerShip/RESTAPI_WEB';
    ...
    try {
      ...
      // Add quote type to additional information if it's not there
      if (!correctedBody.AdditionalInformation) {
        correctedBody.AdditionalInformation = "Quote Request";
      } else if (!correctedBody.AdditionalInformation.includes("Quote Request")) {
        correctedBody.AdditionalInformation = `Quote Request - ${correctedBody.AdditionalInformation}`;
      }
      ...
      const paramMapping: Record<string, string> = {
        ...
        'Location': correctedBody.Location || 'Website Quote Request',
      ...
      // Make API call - Exactly matching the successful approach in test drive form
      ...
      try {
        ...
        const response = await fetch(apiEndpoint, {
          method: 'POST',
          headers: apiHeaders,
          // Empty body - this matches the working implementation
        });
...
function simulateSuccessResponse(formData: any) {
  ...
  const referenceNumber = `QR-${Math.floor(Math.random() * 100000).toString().padStart(6, '0')}`;
